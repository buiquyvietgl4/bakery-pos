...
-- Bao gồm: 16 file migration (Từ 00001 đến 00016) - CÓ CẢ TẠO KHO ẢNH STORAGE!
-- Hướng dẫn: Mở SQL Editor trên Supabase, copy toàn bộ nội dung file này,
--            dán vào và bấm nút [RUN] để tạo 100% bảng, hàm, views và kho ảnh.
-- =====================================================================
...
-- >>> BẮT ĐẦU MIGRATION: 00016_create_storage_buckets.sql <<<
...
-- >>> BẮT ĐẦU MIGRATION: 00018_material_stock_adjustments.sql <<<
-- 00018_material_stock_adjustments.sql
-- Bảng lưu lịch sử điều chỉnh / kiểm kê tồn kho nguyên vật liệu
...
ALTER TABLE public.material_stock_adjustments REPLICA IDENTITY FULL;

DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_publication_tables WHERE pubname = 'supabase_realtime' AND tablename = 'material_stock_adjustments') THEN
