...
export default function LocalSqlConfigSection() {
  ...
  useEffect(() => {
    ...
    const defaultPath = selectedEnvId === 'production'
      ? 'C:\\Users\\H\\.gemini\\antigravity\\scratch\\bakery-erp\\SQL backup\\SQL LOCAL'
      : 'C:\\Users\\H\\.gemini\\antigravity\\scratch\\bakery-erp\\SQL backup\\SQL TEST';

    if (envConfig) {
      const activePath = envConfig.folderPath || (envConfig.folderName === 'SQL LOCAL' ? defaultPath : '');
      if (activePath) {
        setServerDirPathInput(activePath);
        ...
        fetch(`/api/local-sql?find_name=${encodeURIComponent(envConfig.folderName)}`)
          ...
          .then((d) => {
            if (d.success && d.path) {
              ...
              setServerDirPathInput(defaultPath);
            }
          })
          .catch(() => {
            setServerDirPathInput(defaultPath);
  ...
  const fallbackPath = selectedEnvId === 'production'
    ? 'C:\\Users\\H\\.gemini\\antigravity\\scratch\\bakery-erp\\SQL backup\\SQL LOCAL'
    : 'C:\\Users\\H\\.gemini\\antigravity\\scratch\\bakery-erp\\SQL backup\\SQL TEST';
  const resolvedDisplayPath = currentEnvConfig.folderPath || serverDirPathInput || fallbackPath;
  ...
  const handleApplyPath = async () => {
    const trimmed = (serverDirPathInput || resolvedDisplayPath).trim();
    if (!trimmed) {
  ...
  const handleTestFolder = async () => {
    const targetPath = (serverDirPathInput || resolvedDisplayPath || currentEnvConfig.folderPath || '').trim();
    if (!targetPath) {
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-3xl border border-zinc-200 p-4 sm:p-6 shadow-xs space-y-4">
        ...
        <div className="space-y-3">
          ...
          {/* THẺ HIỂN THỊ ĐƯỜNG DẪN THƯ MỤC CSDL ĐẦY ĐỦ VÀ RÕ RÀNG */}
          <div className="p-4 rounded-2xl border-2 transition-all flex flex-col gap-2.5 bg-emerald-50/90 border-emerald-300 shadow-2xs">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                <div className="w-7 h-7 rounded-xl bg-emerald-600 text-white flex items-center justify-center font-black shadow-2xs">
                  <Folder className="w-4 h-4" />
                </div>
                <span className="text-xs font-black text-emerald-950 uppercase tracking-wide">
                  ĐƯỜNG DẪN THƯ MỤC CSDL ĐANG CHỌN ({selectedEnvId === 'production' ? 'CHÍNH' : 'TEST'}):
                </span>
              </div>
              <div className="flex items-center gap-2">
                <span className="px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-emerald-200 text-emerald-900 border border-emerald-300 flex items-center gap-1">
                  <Check className="w-3.5 h-3.5 text-emerald-700" />
                  <span>Đã Kết Nối Thư Mục</span>
                </span>
                {currentEnvConfig.lastSyncAt && (
                  <span className="text-[11px] text-emerald-800 font-medium">
                    (Cập nhật: {new Date(currentEnvConfig.lastSyncAt).toLocaleTimeString('vi-VN')})
                  </span>
                )}
              </div>
            </div>

            {/* Khung hiển thị đường dẫn to rõ ràng */}
            <div className="flex items-center justify-between gap-2 p-2.5 bg-white rounded-xl border border-emerald-300 shadow-2xs">
              <div className="flex items-center gap-2 min-w-0 font-mono text-xs font-bold text-zinc-900 break-all select-all">
                <HardDrive className="w-4 h-4 text-emerald-600 shrink-0" />
                <span>{resolvedDisplayPath}</span>
              </div>
              <button
                type="button"
                onClick={() => {
                  navigator.clipboard.writeText(resolvedDisplayPath);
                  showNotice('success', 'Đã sao chép đường dẫn: ' + resolvedDisplayPath);
                }}
                className="px-2.5 py-1 text-[11px] font-bold text-emerald-800 bg-emerald-50 hover:bg-emerald-100 rounded-lg border border-emerald-200 flex items-center gap-1 transition cursor-pointer shrink-0"
          ...
          <div className="p-3 bg-zinc-50 rounded-2xl border border-zinc-200 flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
            ...
            <input
              type="text"
              value={serverDirPathInput || resolvedDisplayPath}
              onChange={(e) => setServerDirPathInput(e.target.value)}
              placeholder={resolvedDisplayPath}
              className="flex-1 px-3 py-1.5 text-xs bg-white border border-zinc-300 rounded-xl focus:outline-hidden focus:ring-1 focus:ring-amber-500 font-mono font-bold text-zinc-900"
            />
            <button
              type="button"
              onClick={handleApplyPath}
              disabled={isSyncing || !(serverDirPathInput || resolvedDisplayPath).trim()}
            ...
            <button
              ...
              disabled={isTestingFolder || !(serverDirPathInput || resolvedDisplayPath).trim()}
