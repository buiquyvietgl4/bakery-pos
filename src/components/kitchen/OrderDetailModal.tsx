    ...
    <div className={`fixed inset-0 ${zIndexClass || 'z-[10000005]'} bg-black/80 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4 overflow-y-auto animate-in fade-in duration-150`}>
      <div className="bg-zinc-900 border border-zinc-700/80 rounded-3xl max-w-lg w-full p-5 sm:p-6 shadow-2xl space-y-4 text-white animate-in zoom-in-95 duration-150 my-auto">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className={`text-[10px] font-extrabold px-2 py-0.5 rounded-md border ${
                ...
                {isBirthdayCake ? '🎂 BÁNH SINH NHẬT' : isPreorder ? '📦 ĐƠN ĐẶT TRƯỚC' : '🏪 BÁN TẠI QUẦY'}
              </span>
        ...
        <div className="space-y-3.5 max-h-[68vh] overflow-y-auto pr-1">
          ...
          {/* 1. KHỐI THÔNG TIN BÁNH ĐẶT HÀNG */}
          {!isBirthdayCake ? (
            <div className="bg-zinc-950/90 rounded-2xl p-4 border border-zinc-800/90 space-y-3">
              <div className="text-[11px] font-bold text-zinc-400 uppercase tracking-wider flex items-center justify-between">
                <span className="flex items-center gap-1.5">
                  <Package className="w-3.5 h-3.5 text-amber-400" /> Danh sách món đặt hàng ({order.items?.length || 1})
                </span>
                {onViewBom && (
                  <button
                    type="button"
                    onClick={() => onViewBom(order)}
                    className="px-2.5 py-1 rounded-xl bg-amber-500/20 hover:bg-amber-500/30 border border-amber-500/50 text-amber-300 hover:text-white text-[11px] font-bold flex items-center gap-1 transition cursor-pointer active:scale-95 shadow-xs"
              ...
              <div className="space-y-2 divide-y divide-zinc-800/60">
                {(order.items && order.items.length > 0 ? order.items : [mainItem]).map((it: any, idx: number) => {
                  const itName = it?.product_name_snapshot || it?.product?.name || it?.name || cakeName;
                  const itQty = it?.quantity || 1;
                  return (
                    <div key={idx} className={idx > 0 ? 'pt-2' : ''}>
                      <div className="flex items-center justify-between gap-2">
                        <span className="font-bold text-sm text-zinc-100">
                          <span className="text-amber-400 font-black mr-2">{itQty}x</span>
                          {itName}
                        </span>
                      </div>
                      {it?.notes && (
              ...
              {specialReq && (
                <div className="pt-2 border-t border-zinc-800/80">
                  <div className="text-xs bg-amber-950/30 p-2.5 rounded-xl border border-amber-800/50 text-amber-200">
                    <span className="text-amber-400 font-bold text-[11px] block">Ghi chú của khách:</span>
          ...
          {/* 1.5. KHỐI CỐT BÁNH, KEM, HỘP & PHỤ KIỆN ĐẶT THÊM (CHỈ DÀNH CHO BÁNH SINH NHẬT) */}
          {isBirthdayCake && (flavor || cream || packaging || allAddons.length > 0) && (
