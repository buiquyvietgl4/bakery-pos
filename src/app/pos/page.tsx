...
export default function POSPage() {
  ...
  useEffect(() => {
    ...
    const unsubscribeSync = subscribeCrossDeviceSync({
      ...
      onSystemGlobalWipe: async (payload) => {
        ...
        if (payloadEpoch > 0 && payloadEpoch <= localEpoch) {
          console.log('ℹ️ [POS] Bỏ qua lệnh reset đã xử lý hoặc cũ hơn epoch hiện tại:', payloadEpoch);
          return;
        }
        if (typeof window !== 'undefined') {
          const reloadedEpoch = sessionStorage.getItem('bakery_wiped_reloaded_epoch');
          if (reloadedEpoch && Number(reloadedEpoch) === payloadEpoch) {
            console.log('ℹ️ [POS] Tab đã reload cho epoch này rồi, dừng vòng lặp:', payloadEpoch);
            return;
          }
        }
    ...
    const handleSystemWiped = async (e: any) => {
      ...
      if (isPosWiping || (window as any).__IS_RELOADING_FOR_WIPE__) return;
      isPosWiping = true;
      (window as any).__IS_RELOADING_FOR_WIPE__ = true;
                    ...
                    >
                      {cakeCostingConfig.sizes.map((s) => (
                        <option key={s.id} value={s.id}>
                          {s.name} — Vốn ~{(Number(s.baseCost) || 0).toLocaleString('vi-VN')}₫ (Gợi ý: {(Number(s.suggestedPrice) || 0).toLocaleString('vi-VN')}₫)
                        </option>
                      ))}
                    </select>
                  </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className={`text-[10px] font-black mt-1 ${isSelected ? 'text-white' : 'text-pink-700'}`}>
                            {(Number(s.suggestedPrice) || 0).toLocaleString('vi-VN')}₫
                          </div>
                        </button>
                      );
                    })}
                    ...
                    >
                      {cakeCostingConfig.flavors.map((f) => (
                        <option key={f.id} value={f.id}>
                          {f.name} {(Number(f.extraPrice) || 0) > 0 ? `(+${(Number(f.extraPrice) || 0).toLocaleString('vi-VN')}₫)` : ''}
                        </option>
                      ))}
                    </select>
                  </div>
                    ...
                    >
                      {cakeCostingConfig.creams.map((c) => (
                        <option key={c.id} value={c.id}>
                          {c.name} {(Number(c.extraPrice) || 0) > 0 ? `(+${(Number(c.extraPrice) || 0).toLocaleString('vi-VN')}₫)` : ''}
                        </option>
                      ))}
                    </select>
                  </div>
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className={`text-[10px] font-normal mt-0.5 ${isSelected ? 'text-rose-100' : 'text-zinc-500'}`}>
                              {(Number(fl.extraPrice) || 0) > 0 ? `Phụ thu: +${(Number(fl.extraPrice) || 0).toLocaleString('vi-VN')}₫` : 'Mặc định (Đã gồm)'}
                            </div>
                          </button>
                        );
                      })}
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className={`text-[10px] font-normal ${isSelected ? 'text-amber-100' : 'text-zinc-500'}`}>
                            {(Number(pkg.extraPrice) || 0) > 0 ? `Phụ thu: +${(Number(pkg.extraPrice) || 0).toLocaleString('vi-VN')}₫` : 'Tiêu chuẩn (Đã gồm)'}
                          </div>
                        </button>
                      );
                    })}
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className={`text-[10px] ${isSelected ? 'text-pink-100 font-bold' : 'text-rose-600'}`}>
                            {(Number(a.price) || 0) > 0 ? `+${(Number(a.price) || 0).toLocaleString('vi-VN')}₫` : 'Free'}
                          </span>
                        </button>
                      );
                    })}
                ...
                {isAdmin ? (
                  <div className="p-3 bg-white/95 rounded-xl border-2 border-pink-300 shadow-xs space-y-2 text-xs animate-in fade-in duration-200">
                    ...
                    <div className="grid grid-cols-3 gap-2 bg-pink-50/50 p-2 rounded-lg border border-pink-100 text-center">
                      <div>
                        ...
                        <span className="text-xs font-black text-rose-600">
                          {(Number(cakeCostResult?.totalCost) || 0).toLocaleString('vi-VN')}₫
                        </span>
                      </div>
                      <div>
                        ...
                        <span className="text-xs font-black text-pink-700">
                          {(Number(cakeCostResult?.suggestedPrice) || 0).toLocaleString('vi-VN')}₫
                        </span>
                      </div>
                      <div>
                        <span className="text-[10px] text-zinc-500 block">Lãi Gộp Dự Kiến</span>
                        <span className="text-xs font-black text-emerald-600">
                          +{(Number(cakeCostResult?.estimatedProfit) || 0).toLocaleString('vi-VN')}₫
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-2 pt-1 border-t border-zinc-100">
                      ...
                      >
                        <Sparkles className="w-3 h-3" /> Điền Giá Gợi Ý ({(Number(cakeCostResult?.suggestedPrice) || 0).toLocaleString('vi-VN')}₫)
                      </button>
                    </div>
                  </div>
                ) : (
                  ...
                  <div className="p-3 bg-pink-50/70 rounded-xl border border-pink-200 shadow-2xs space-y-2 text-xs">
                    ...
                    <div className="flex justify-end pt-1 border-t border-pink-200/50">
                      ...
                      >
                        <Sparkles className="w-3.5 h-3.5" /> Điền Giá Gợi Ý Này ({(Number(cakeCostResult?.suggestedPrice) || 0).toLocaleString('vi-VN')}₫)
                      </button>
                    </div>
                  </div>
                )}
                            ...
                            return (
                              <div key={idx} className="flex justify-between items-center py-1 text-zinc-800 gap-2">
                                ...
                                <span className={remainingQty === 0 && totalThisItemReturned > 0 ? "font-bold line-through text-zinc-400 shrink-0" : "font-bold text-zinc-900 shrink-0"}>
                                  {((Number(itemUnitPrice) || 0) * (Number(itOriginalQty) || 0)).toLocaleString('vi-VN')}₫
                                </span>
                              </div>
                            );
                          })
                              ...
                              <div className="flex items-center gap-1.5">
                                ...
                                {invRefundedAmount > 0 && (
                                  <span className="font-bold text-rose-700 ml-1">
                                    • Hoàn: -{(Number(invRefundedAmount) || 0).toLocaleString('vi-VN')}₫
                                  </span>
                                )}
                              </div>
                            ...
                            {isExpanded && (
                              <div className="bg-white p-2.5 rounded-xl border border-rose-200 shadow-xs space-y-2 text-xs animate-in fade-in-50">
                                <div className="text-[11px] font-bold text-rose-900 border-b border-rose-100 pb-1 flex justify-between">
                                  ...
                                  <span className="font-mono text-rose-700">Tổng hoàn: -{(Number(invRefundedAmount) || 0).toLocaleString('vi-VN')}₫</span>
                                </div>
                                <div className="space-y-2">
                                    ...
                                    return (
                                      <div key={r.id || rIdx} className="bg-stone-50/70 p-2 rounded-lg border border-stone-200/80 space-y-1">
                                            ...
                                            {r.exchange_difference !== undefined && (
                                              <div className="flex justify-between text-[11px] font-bold pt-0.5 text-zinc-900">
                                                ...
                                                <span className={r.exchange_difference > 0 ? "text-emerald-700" : "text-rose-600"}>
                                                  {Math.abs(Number(r.exchange_difference) || 0).toLocaleString('vi-VN')}₫ ({rMethodStr})
                                                </span>
                                              </div>
                                            )}
                                          </div>
                                        ...
                                        <div className="flex flex-wrap items-center justify-between text-[10px] text-zinc-500 pt-0.5 border-t border-stone-200">
                                          ...
                                          {r.refund_amount > 0 && (
                                            <span className="font-bold text-rose-600">
                                              Đã chi hoàn: {(Number(r.refund_amount) || 0).toLocaleString('vi-VN')}₫ ({rMethodStr})
                                            </span>
                                          )}
                                        </div>
                                      </div>
                        ...
                        <div className="flex flex-wrap items-center gap-3">
                          ...
                          {invRefundedAmount > 0 && (
                            <span className="text-zinc-700">
                              Thực thu: <b className="text-emerald-700">{(Math.max(0, (Number(total) || 0) - (Number(invRefundedAmount) || 0))).toLocaleString('vi-VN')}₫</b>
                            </span>
                          )}
                          {deposit > 0 && (
                    ...
                    <div className={`grid grid-cols-1 ${isAdmin ? 'sm:grid-cols-3' : 'sm:grid-cols-2'} gap-2.5`}>
                      ...
                      {isAdmin && (
                        <div className="bg-amber-50 p-3 rounded-2xl border border-amber-200">
                          ...
                          <span className="text-lg font-black text-amber-900">
                            {(Number(summary.totalCostLoss) || 0).toLocaleString('vi-VN')}₫
                          </span>
                        </div>
                      )}
                      <div className="bg-zinc-50 p-3 rounded-2xl border border-zinc-200">
                        ...
                        <span className="text-lg font-black text-zinc-700">
                          {(Number(summary.totalRevenueLoss) || 0).toLocaleString('vi-VN')}₫
                        </span>
                      </div>
                    </div>
                  );
                        ...
                        <div className="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-end">
                          {isAdmin && (
                            <span className="text-xs font-bold text-zinc-600">
                              Thiệt hại vốn: <b className="text-rose-600 text-sm">{(Number(costLoss) || 0).toLocaleString('vi-VN')}₫</b>
                            </span>
                          )}
                          <button
                            type="submit"
              ...
              return (
                <div className="p-3.5 bg-rose-50/70 rounded-2xl border border-rose-200/80 space-y-2 text-xs">
                  ...
                  <div className="flex justify-between items-center pt-1 border-t border-rose-200/60">
                    ...
                    <span className="font-black text-rose-700 text-base">{(Number(totalAmt) || 0).toLocaleString('vi-VN')}₫</span>
                  </div>
                </div>
              );
            })()}
